        ...
        <div className="event-left py-2">
          <div className="event-details p-0 my-2 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <h1 className="py-2">
                {event?.eventName ? event.eventName : "Loading..."}
              </h1>
              <img
          ...
          <p><strong>Category: </strong>{event ? capitalizedStr(event.eventCategory) : "Loading..."}</p> */}
              ...
              {event ? (
                <Link
                  to={
                    ...
                    event.user +
                    "&name=" +
                    encodeURIComponent(event.hostName)
                  }
                >
                  {event.hostName}
                </Link>
            ...
            <p>
              ...
              {event ? formatNames(event.sponserName) : " Loading..."}
            </p>
            <p>
              <strong>Performer Name : </strong>{" "}
              {event ? formatNames(event.performerName) : " Loading..."}
            </p>
            <p>
              <strong>
                Contact Details <WhatsApp sx={{ color: "#D26600" }} /> :{" "}
              </strong>{" "}
              {event ? `+91-${event.hostWhatsapp}` : " Loading..."}
            </p>
            <p>
              <strong>Language : </strong>{" "}
              {event ? capitalizedStr(event.eventLang) : "Loading..."}
            </p>
            <p>
        ...
        <div className="event-right pb-0 mq750:!p-0">
          <div className="event-meta flex flex-col items-end gap-[10px] mq750:items-center">
            <div className="interest">
              ...
              <span>
                {" "}
                {event ? getTotalAttendees(event.bookings) : "0"} Already
                Interested
              </span>
            </div>
          ...
          <div className="event-map text-start" style={{ color: "#D26600" }}>
            ...
            <p>
              <Pin sx={{ color: "#D26600" }} />{" "}
              <strong>{event?.eventAddress}</strong>
            </p>
            <div className="map-container">
              ...
              {event ? (
                <EventMap
                  center={{
                    lat: event.geoCoordinates.coordinates[1],
                    lng: event.geoCoordinates.coordinates[0],
                  }}
                />
              ) : (
                <h1>Loading...</h1>
